...
from ValDX.helpful_funcs import PDB_to_DSSP, segs_to_df
...
class AnalysisData:
    ...
    def verify_dataframe(self, df):
        ...
        df["protein"] = [
            i.split("_")[3] if len(i.split("_")) > 3 else "Experiment" for i in df["name"]
        ]
...
class PeptideSplitter:
    ...
    def __init__(
        ...
        assert isinstance(self.expt_segs_df, pd.DataFrame), (
            "expt_segs_df must be a pandas DataFrame"
        )
        self.keys = keys
    ...
    def peptide_centrality_split(
        self, split_fraction: float = None, handle_intersection: bool = False
    ):
        """
        ...
        if split_fraction == 0:
            print("Train fraction is 0, returning all peptides for train and val.")
            all_peps = self.expt_segments.pep_nums
            return all_peps, all_peps
    ...
    def drop_centrality(self, split_fraction: float = 0.9, drop: bool = True):
        if drop:
            _, val_peps = self.peptide_centrality_split(
                split_fraction=split_fraction, handle_intersection=False
            )
    ...
    def validate_split(
        self, train_peps: np.ndarray, val_peps: np.ndarray, lower_min_pep_threshold: float = 0.1
    ):
    ...
    def random_split(
        ...
        all_peps = new_expt_segs.pep_nums
        ...
        train_peps = np.random.choice(all_peps, size=int(len(all_peps) * train_frac), replace=False)
        val_peps = np.setdiff1d(all_peps, train_peps)
    ...
    def sequence_split(
        ...
        expt_segments_df = self.drop_centrality(
            drop=drop_centrality
        )  # should also increase this as well to improve randomness of split
        new_expt_segs = Segments(expt_segments_df, keys=self.keys)
        all_peps = new_expt_segs.pep_nums
        ...
        print(f"Drop {drop} and hard_intersection {hard_intersection}")
        sequence_pep = int(len(all_peps) * train_frac)

        train_peps = all_peps[:sequence_pep]
        val_peps = all_peps[sequence_pep:]
        ...
        if drop:
            train_peps, val_peps = drop_intersection(
                new_expt_segs, train_peps, val_peps, hard=hard_intersection
            )
            if self.validate_split(train_peps, val_peps):
                return train_peps, val_peps
            else:
                print("Split not valid, recalculating")
                return self.sequence_split(
    ...
    def redundant_sequence_split(
        ...
        if self.validate_split(train_peps, val_peps):
            ...
            print("Split not valid, recalculating")
            return self.redundant_sequence_split(
                train_frac=train_frac,
                drop_centrality=drop_centrality,
                hard_intersection=hard_intersection,
            )
    ...
    def structural_split(
        ...
        print(
            f"Structural split with train_frac {train_frac} and drop_centrality {drop_centrality}"
        )
        print(f"and hard_intersection {hard_intersection}")
        ...
        helix_peptides = new_expt_segs.df_select_residues(list(helix_residues))[
            "peptide"
        ].to_numpy()
        sheet_peptides = new_expt_segs.df_select_residues(list(sheet_residues))[
            "peptide"
        ].to_numpy()
        ...
        if compare:
            ...
            _peptide_splitter = PeptideSplitter(
                expt_segs_df=new_expt_segs.segs_df,
                train_frac=train_frac,
                random_seed=self.random_seed,
            )
            return _peptide_splitter.redundant_sequence_split(
                train_frac=train_frac, drop_centrality=False, hard_intersection=hard_intersection
            )

    def neighbours_split(
        self,
        top_path: str,
        train_frac: float = None,
        drop_centrality: bool = True,
        ...
        print(
            f"Neighbours split with train_frac {train_frac} and drop_centrality {drop_centrality}"
        )
        print(f"and hard_intersection {hard_intersection}")
        ...
        residue_selection_string = " or ".join(
            [f"(resnum {residue} and name CA)" for residue in HDX_residues]
        )
    ...
    def spatial_split(
        ...
        expt_segments_df = self.drop_centrality(
            drop=True
        )  # perhaps we can increase the drop_centrality %
        new_expt_segs = Segments(expt_segments_df, keys=self.keys)
        all_peps = new_expt_segs.pep_nums
        ...
        if kmeans_cluster:
            ksplits = 10
            kmeans = KMeans(n_clusters=ksplits, random_state=self.random_seed).fit(
                pca_coords
            )  # weight kmeans by peptide centrality
        ...
        train_peps, val_peps = drop_intersection(
            new_expt_segs, train_peptides, val_peptides, hard=hard_intersection
        )
        if self.validate_split(train_peps, val_peps):
            return train_peps, val_peps
        else:
...
def drop_intersection(
    expt_segs: Segments, train_peps: np.ndarray, val_peps: np.ndarray, hard: bool = False
):
    """
    ...
    intersection_peptides = expt_segs.df_select_residues(list(train_val_intersection))[
        "peptide"
    ].to_numpy()
...
def create_train_val_dfs(
    expt_segs_df: pd.DataFrame,
