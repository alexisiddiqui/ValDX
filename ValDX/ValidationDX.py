...
from ValDX.VDX_dataclasses import AnalysisData, AnalysisInfo, Segments, merge_AnalysisData_classes
from ValDX.VDX_Settings import Settings


class ValDXer(Experiment):
    def __init__(
        self, settings: Settings, name=None, overwrite=False, analysis_name: List[str] = None
    ):
        ...
        self.initialise_dir_structure(
            prefix=self.analysis_name, paths_only=True, overwrite_output=overwrite
        )
    ...
    def load_HDX_data(
        ...
        if HDX_path and SEG_path is not None:
            ...
            paths_to_add = pd.DataFrame(
                {"SEG": [SEG_path], "calc_name": [calc_name], "experimental": [experimental]}
            )
        elif SEG_path is None:
            paths_to_add = pd.DataFrame(
                {"HDX": [HDX_path], "calc_name": [calc_name], "experimental": [experimental]}
            )
    ...
    def load_structures(self, top_path, traj_paths: list = None, calc_name: str = None):
        """
        ...
        paths_to_add = pd.DataFrame(
            {"top": [top_path], "traj": [traj_paths], "calc_name": [calc_name]}
        )
        self.paths = pd.concat([self.paths, paths_to_add], ignore_index=True)
        print(self.paths)
    ...
    def featurise_HDX(
        ...
        args = super().featurise_HDX(calc_name=calc_name, rep=rep, train=train)
        ...
        self.load_intrinsic_rates(out_prefix + "Intrinsic_rates.dat", calc_name=rep_name)

        return df, rep_name, out_dir

    def featurise_HDX_all_reps(self, calc_name: str = None, train: bool = True, n_reps: int = None):
        ...
        for arg in args_list:
            self.load_intrinsic_rates(
                arg["out_prefix"] + "Intrinsic_rates.dat", calc_name=arg["rep_name"]
            )
    ...
    def reweight_HDX(
        ...
        if (expt_name or calc_name) is None:
            raise ValueError(
                "Please provide a Calculation AND Experimental name for the structures."
            )
        ...
        if predictHDX_dir is None:
            predictHDX_dir, _, _, _ = self.create_path_str(
                prefix=self.analysis_name, calc_name=rep_name
            )  # change this to use the training HDX data
        # expt = self.paths.loc[self.paths["calc_name"] == expt_name, "HDX"].dropna().values[0]
        if train:
            expt = (
                self.train_HDX_data.loc[self.train_HDX_data["calc_name"] == rep_name, "path"]
                .dropna()
                .values[0]
            )
        else:
            expt = (
                self.val_HDX_data.loc[self.val_HDX_data["calc_name"] == rep_name, "path"]
                .dropna()
                .values[0]
            )
    ...
    def reweight_train_ensemble(
        ...
        assert np.round(np.sum(_weights)) == 1, (
            f"when saving input weights, they must sum to 1 so they are compatible with HDXer: {np.sum(_weights)}"
        )
        # add input weights and BV as expt_name

        weights_to_add = pd.DataFrame({"weights": [_weights], "calc_name": ["prior_" + calc_name]})

        self.weights = pd.concat([self.weights, weights_to_add], ignore_index=True)

        params_to_add = pd.DataFrame(
            {"Bc": [bc_bh[0]], "Bh": [bc_bh[1]], "calc_name": ["prior_" + calc_name]}
        )

        self.BV_constants = pd.concat([self.BV_constants, params_to_add], ignore_index=True)

        args_list = self.generate_MaxEnt_trainval_params(
            train=True,
            n_reps=n_reps,
            calc_name=calc_name,
            expt_name=expt_name,
        ...
        for idx, _args_list in enumerate(args_list):
            ...
            _gamma_list = zip(
                [args["r"] for args in _args_list], [args["exponent"] for args in _args_list]
            )
            # find the correct index from _gamma_list
            gamma_index = [
                idx
                for idx, gamma in enumerate(_gamma_list)
                if gamma == (opt_gamma_coefficient, opt_gamma_exponent)
            ][0]
    ...
    def generate_MaxEnt_trainval_params(
        self,
        train: bool = True,
        n_reps: int = None,
        calc_name: str = None,
        ...
        if weights is not None:
            assert np.round(np.sum(weights)) == len(weights), (
                "input weights must sum to length"
            )  # check this is true
        ...
        if train:
            ...
            base_args_list = self.generate_MaxEnt_params(
                prefix_name="train",
                n_reps=n_reps,
                calc_name=calc_name,
                expt_name=expt_name,
    ...
    def generate_MaxEnt_params(
        ...
        for rep_name in rep_names:
            rate = (
                self.paths.loc[self.paths["calc_name"] == rep_name, "int_rates"]
                .dropna()
                .to_list()[0]
            )
            rates.append(rate)
    ...
    def recalculate_dataset(
        ...
        assert np.round(np.sum(weights)) == 1, (
            f"weights must sum {np.round(np.sum(weights))} to 1 length: {len(weights)}"
        )

        LogPf_by_res = calc_ave_lnpi(
            contacts=contacts, hbonds=hbonds, weights=weights, bc=bv_bc, bh=bv_bh
        )

        assert len(LogPf_by_res) == len(rates.keys()), (
            f"LogPfs must be the same length {len(LogPf_by_res)} as the number of residues {len(rates.keys())}"
        )
        ...
        if test:
            # calculate likelihood using PMF of residue average detla G using confromational enthalpy and entropy of the LogPfs per residue
            prior_name = dataset_name.replace("test", "prior")

            prior_LogPfs = self.LogPfs[self.LogPfs["calc_name"] == prior_name]["LogPf"].values

            likelihood = calc_likelihood_from_LogPfs(
                prior_LogPfs=prior_LogPfs, test_LogPfs=residue_LogPfs
            )

            # add to weights df
            weights_to_add = pd.DataFrame(
                {"likelihood": [likelihood], "calc_name": [dataset_name.replace("test", "train")]}
            )
        ...
        if not train:
            ...
            dfracs_by_res_overtime = calc_dfrac_uptake_from_LogPf(
                residue_LogPfs, kints=rates, times=self.settings.times, residues=residues
            )
            ...
            for i, p in enumerate(peptides.keys()):
                ...
                peptide_indices = [
                    idx for idx, res in enumerate(residues) if res in peptide_residues
                ]
    ...
    def recalculate_test_and_val(self, cr_bc_bh, predictHDX_dir, calc_name, expt_name, rep=None):
        ...
        params_to_add = pd.DataFrame(
            {"Bc": cr_bc_bh[1], "Bh": cr_bc_bh[2], "calc_name": [rep_name]}
        )
        self.BV_constants = pd.concat([self.BV_constants, params_to_add], ignore_index=True)
        ...
        expt_weights = self.weights[self.weights["calc_name"] == "prior_" + calc_name][
            "weights"
        ].values[0]
        bv_bc = self.BV_constants[self.BV_constants["calc_name"] == "prior_" + calc_name][
            "Bc"
        ].values[0]
        bv_bh = self.BV_constants[self.BV_constants["calc_name"] == "prior_" + calc_name][
            "Bh"
        ].values[0]

        expt_cr_bc_bh = (expt_weights, bv_bc, bv_bh)

        prior_df = self.recalculate_dataset(
            traj=traj,
        ...
        val_df = self.recalculate_dataset(
            traj=traj,
            cr_bc_bh=cr_bc_bh,
            predictHDX_dir=predictHDX_dir,
        ...
        test_df = self.recalculate_dataset(
            traj=traj,
            cr_bc_bh=cr_bc_bh,
            expt_name=expt_name,
            predictHDX_dir=predictHDX_dir,
    ...
    def write_data_split_PDB(self, calc_name, expt_name, rep):
        ...
        train_segs["residues"] = train_segs.apply(
            lambda x: list(range(x["ResStr"] + 1, x["ResEnd"] + 1)), axis=1
        )
        val_segs["residues"] = val_segs.apply(
            lambda x: list(range(x["ResStr"] + 1, x["ResEnd"] + 1)), axis=1
        )
    ...
    def write_RW_representative_PDB(self, calc_name, rep, weights, cluster_size2=None):
        """
    ...
    def run_VDX(
        ...
        for rep in range(1, n_reps + 1):
            _, train_rep_name, val_rep_name = self.split_segments(
                seg_name=expt_name,
                calc_name=calc_name,
                rep=rep,
                mode=mode,
        ...
        _, _, predictHDX_dirs, _ = self.featurise_HDX_all_reps(
            calc_name=calc_name, n_reps=n_reps, train=True
        )

        # print(bc_bh)
        # raise ValueError("DEBUGGING")
        train_gammas, train_dfs, cr_bc_bhs = self.reweight_train_ensemble(
            n_reps=n_reps,
            calc_name=calc_name,
            expt_name=expt_name,
            predictHDX_dirs=predictHDX_dirs,
        ...
        for idx, rep in enumerate(list(range(1, n_reps + 1))):
            ...
            predictHDX_dir = predictHDX_dirs[idx]
            val_opt_gamma, val_df, test_df = self.validate_HDX(
                calc_name=calc_name,
                expt_name=expt_name,
                mode=mode,
                rep=rep,
            ...
            if self.settings.plot:
                self.write_data_split_PDB(calc_name=calc_name, expt_name=expt_name, rep=rep)
                # if (self.settings.RW_do_reweighting is True):
    ...
    def run_cluster_benchmark_ensemble(
        self,
        system: str = None,
        times: list = None,
        expt_name: str = None,
        ...
        if n_clusters == 0:
            n_clusters = u.trajectory.n_frames
        ...
        assert len(weights) == u.trajectory.n_frames, (
            f"weights must be the same length {len(weights)} as the number of frames {u.trajectory.n_frames}"
        )

        projected = PCA_universe(u)

        cluster_frames, final_cluster_weights, cluster_centers, cluster_labels = (
            recluster_traj_by_weight(
                projected=projected, cluster_weights=weights, cluster_size2=n_clusters
            )
        )
        ...
        data, names, save_path = self.run_benchmark_ensemble(
            system=system,
            times=times,
            expt_name=expt_name,
            n_reps=n_reps,
        ...
        weights_data = weights_data[weights_data["calc_name"].str.contains("train")]

        # final_weights = weights_data["weights"].values

        final_weights = np.array([np.array(w) for w in weights_data["weights"].dropna().values])
        print(f"Final Weights: {final_weights}")  # average weights
    ...
    def run_benchmark_ensemble(
        ...
        split_benchmark_plot_MSE_by_split(
            MSE_df, save=settings.save_figs, save_dir=save_dir, title_str=system
        )
        ...
        if BV:
            ...
            split_benchmark_BV_boxplot_by_split_type(
                BV_df, save=settings.save_figs, save_dir=save_dir, title_str=system
            )
    ...
    def run_refine_ensemble(
        ...
        cluster_frac1 = n_clusters / u.trajectory.n_frames

        cluster_frames, iniweights, projected, cluster_centers, cluster_labels = (
            cluster_traj_by_density(projected=projected, cluster_frac1=cluster_frac1)
        )
        plot_cluster_weights(
            projected_data=projected,
            new_cluster_centers=cluster_centers,
        ...
        # save clustered universe
        clustered_traj_name = "_".join([system, "clustered", "cfrac1", str(cluster_frac1), ".xtc"])
        clustered_traj_path = os.path.join(self.data_dir, clustered_traj_name)
        ...
        # data, names, save_paths = self.run_benchmark_ensemble(system=system+"_cl",
        #                                     times=times,
        #                                     expt_name=expt_name,
        #                                     n_reps=n_reps,
        #                                     hdx_path=hdx_path,
        ...
        # # extract weights from benchmark ensemble
        # weights_df = data["weights"]
        # print(weights_df)
        # weights_df = weights_df[weights_df["calc_name"].str.contains("train")]
        # final_weights = np.array([np.array(w) for w in weights_df["weights"].dropna().values])
        # print(f"Final Weights: {final_weights}")

        # # likelihoods = np.array(weights_df["likelihood"].dropna().values)
        # # print(f"Likelihoods: {likelihoods}")
        # # final_weights = [w*lh for w,lh in zip(final_weights, likelihoods)]
        # # print(f"Final Weights: {final_weights}")
        # avg_weights = np.mean(final_weights, axis=0)
        ...
        # find BV parameters
        # bc_bh = self.find_BV_parameters(system=system+"_cl",
        #                                 times=times,
        #                                 expt_name=expt_name,
        #                                 n_reps=n_reps*5,
        #                                 hdx_path=hdx_path,
        ...
        # print(f"Found Optimal BV Parameters: {bc_bh}")
        bc_bh = 0.35, 2.0
        # reweight trajectory
        ...
        settings.random_seed = settings.random_seed**2

        _VDX = ValDXer(
            settings=settings, name=system + "_refine", analysis_name=["Refine-Ensemble", "RW"]
        )

        _VDX.load_HDX_data(
            HDX_path=hdx_path, SEG_path=segs_path, calc_name=expt_name, experimental=True
        )
        _VDX.load_structures(
            top_path=top_path,
            traj_paths=[clustered_traj_path],
            calc_name=system + "_cl" + "_refine",
        )
        _VDX.settings.random_initialisation = False
        _ = _VDX.run_VDX(
            calc_name=system + "_cl" + "_refine",
            expt_name=expt_name,
            # bc_bh=bc_bh,
        ...
        weights_df = _VDX.weights
        print(weights_df)
        # filter: calc_name contains 'train'
        weights_df = weights_df[weights_df["calc_name"].str.contains("train")]
        print(weights_df)
        final_weights = np.array([np.array(w) for w in weights_df["weights"].dropna().values])
        print(f"Final Weights: {final_weights}")
        ...
        # # normalise likelihoods to sum to 1
        ...
        # normalise the weights to sum to length of weights
        avg_weights = avg_weights * len(avg_weights) / np.sum(avg_weights)

        plot_cluster_weights(
            projected_data=projected[cluster_frames],
            new_cluster_centers=cluster_centers,
        ...
        if not modal_cluster:
            print("Mean Cluster")
            recluster_frames, final_cluster2_weights, reclustered_centers, reclustered_labels = (
                recluster_traj_by_weight(
            ...
            assert len(recluster_frames) == self.settings.cluster_size2, (
                f"Reclustered frames: {len(recluster_frames)} != {self.settings.cluster_size2}"
            )

        elif modal_cluster:
            raise NotImplementedError(
                "Modal Cluster not implemented - this doesnt seem to be a useful feature"
            )
        ...
        reclustered_traj_name = "_".join(
            [system, "reclustered", "csize2", str(self.settings.cluster_size2), ".xtc"]
        )
        ...
        assert reclustered_universe.trajectory.n_frames == len(recluster_frames), (
            f"Reclustered frames: {reclustered_universe.trajectory.n_frames} != {len(recluster_frames)}"
        )

        self.settings.random_initialisation = False

        viz_reclustered_traj_name = "_".join(
            [system, "reclustered", "csize2", str(self.settings.cluster_size2), ".pdb"]
        )
        ...
        return self.run_benchmark_ensemble(
            system=system + "_recl_BVoptimised",
            times=times,
            expt_name=expt_name,
            n_reps=n_reps,
            hdx_path=hdx_path,
    ...
    def train_HDX(
        ...
        if predictHDX_dir is None:
            ...
            _, _, predictHDX_dir = self.featurise_HDX(calc_name=calc_name, rep=rep, train=True)

        # add df to HDX_data

        gamma, df, cr_bc_bh = self.reweight_HDX(
            expt_name=expt_name,
            calc_name=calc_name,
            predictHDX_dir=predictHDX_dir,
            train=True,
    ...
    def train_HDX_all_reps(
        ...
        for rep in range(n_reps):
            if predictHDX_dir is None:
                rep_df, _, predictHDX_dir = self.featurise_HDX(
                    calc_name=calc_name, rep=rep, train=True
                )
    ...
    def validate_HDX(
        ...
        val_df, test_df = self.recalculate_test_and_val(
            cr_bc_bh=cr_bc_bh,
            calc_name=calc_name,
            predictHDX_dir=predictHDX_dir,
            expt_name=expt_name,
    ...
    def evaluate_HDX(
        ...
        expt_segs = self.segs[self.segs["calc_name"] == expt_name].copy()
        merge_df = restore_trainval_peptide_nos(
            calc_name=calc_name,
            expt_name=expt_name,
            train_dfs=train_dfs,
            val_dfs=val_dfs,
        ...
        try:
            ...
            plot_df = plot_R_agreement_trainval(
                expt_name=expt_name,
                train_names=train_names,
                val_names=val_names,
                expt_segs=expt_segs,
        ...
        plot_df = plot_dfracs_compare_MSE(
            args,
            data=nan_df,
            times=self.settings.times,
            save=self.settings.save_figs,
        ...
        if self.settings.plot:
            ...
            train_avg_df = (
                train_df.drop(columns="calc_name").groupby([*times, "peptide"]).mean().reset_index()
            )
            val_avg_df = (
                val_df.drop(columns="calc_name").groupby([*times, "peptide"]).mean().reset_index()
            )
    ...
    def prepare_intrinsic_rates(self, calc_name: str = None):
        ...
        """
        rates_path = (
            self.paths.loc[self.paths["calc_name"] == calc_name, "int_rates"].dropna().values[0]
        )
    ...
    def load_intrinsic_rates(self, path, calc_name: str = None, experimental=False):
        ...
        paths_to_add = pd.DataFrame(
            {"int_rates": [path], "calc_name": [calc_name], "experimental": [experimental]}
        )
        self.paths = pd.concat(
            [self.paths, paths_to_add],
            ignore_index=True,
        )
    ...
    def run_sweep_cluster_ensemble(
        self,
        system: str = None,
        denoms: list = None,
        times: np.array = None,
        ...
        for frac in cluster_fracs:
            cluster_frames, iniweights, _, cluster_centers, cluster_labels = (
            ...
            data, names, save_paths = self.run_benchmark_ensemble(
                system=system + f"_sweep_{str(frac)}",
                times=times,
                expt_name=expt_name,
                n_reps=n_reps,
                hdx_path=hdx_path,
            ...
            for split in split_modes:
                ...
                # select the split_type
                split_df = weights_df[weights_df["split_type"] == split]
                weights_vals = process_weights_df(split_df, filter_contains="train")
                print(weights_vals)
                # average weights
                avg_weights = np.mean(weights_vals, axis=0)
                ...
                assert len(avg_weights) == len(cluster_centers), (
                    f"Length of weights: {len(avg_weights)} != {len(cluster_centers)}"
                )
    ...
    def run_sweep_cluster2_ensemble(
        self,
        system: str = None,
        frames: list = None,
        n_clusters: int = 500,
        ...
        data, names, save_paths = self.run_benchmark_ensemble(
            system=system + f"_ini_{str(frac)}",
            times=times,
            expt_name=expt_name,
            n_reps=n_reps,
            hdx_path=hdx_path,
        ...
        for n_frames in frames:
            ...
            recluster_frames, final_cluster2_weights, reclustered_centers, reclustered_labels = (
                recluster_traj_by_weight(
                    projected=_projected, cluster_weights=ini_avg_weights, cluster_size2=n_frames
                )
            )

            plot_cluster_weights(
                projected_data=_projected,
                new_cluster_centers=reclustered_centers,
                cluster_labels=reclustered_labels,
                new_cluster_weights=final_cluster2_weights,
                save=self.settings.save_figs,
            ...
            data, names, save_paths = self.run_benchmark_ensemble(
                system=system + f"_recl2_{str(n_frames)}",
                times=times,
                expt_name=expt_name,
                n_reps=n_reps,
                hdx_path=hdx_path,
            ...
            # _reclustered_centers = reclustered_centers[recluster_frames]
            assert len(avg_weights) == len(reclustered_centers), (
                f"Length of weights: {len(avg_weights)} != {len(reclustered_centers)}"
            )
            ...
            _ = self.run_benchmark_ensemble(
                system=system + f"_recl2_{str(n_frames)}_{split}",
                times=times,
                expt_name=expt_name,
                n_reps=n_reps,
                hdx_path=hdx_path,
    ...
    def run_sweep_methods(
        ...
        for step_method, method_name in zip(methods, method_names):
            ...
            for idx, method in enumerate(_methods):
                ...
                data, names, save_paths = self.run_benchmark_ensemble(
                    system=system + f"_{method_name}{str(idx)}_fit",
                    times=times,
                    expt_name=expt_name,
                    n_reps=n_reps,
                    hdx_path=hdx_path,
                ...
                for jdx, split in enumerate(_split_modes):
                    ...
                    split_df = weights_df[weights_df["split_type"] == split]
                    weights_vals = process_weights_df(split_df, filter_contains="train")
                    print(f"Weights Values: {weights_vals}")
                    ...
                    for kdx, rep in enumerate(range(n_reps)):
                        ...
                        bench_weights_vals = bench_weights_vals * (
                            len(bench_weights_vals) / np.sum(bench_weights_vals)
                        )
                        ...
                        _ = self.run_benchmark_ensemble(
                            system=system
                            + f"_{method_name}{str(idx)}_bench_{split}_rep_{str(kdx)}",
                            times=times,
                            expt_name=expt_name,
                            n_reps=n_reps,
    ...
    def find_BV_parameters(
        ...
        # run cluster benchmark ensemble
        data, names, save_paths = self.run_benchmark_ensemble(
            system=system + "_BV_fit",
            times=times,
            expt_name=expt_name,
            n_reps=n_reps,
            hdx_path=hdx_path,
        ...
        # join the weights and BV constants on calc_name
        BV_weights = pd.merge(weights_df, BV_df, on="calc_name")

        print(BV_weights.columns)

        # drop entries with prior in calc_name
        BV_weights = BV_weights[~BV_weights["calc_name"].str.contains("prior")]

        # sort by likelihood
        BV_weights = BV_weights.sort_values(by="likelihood", ascending=False)

        # return the highest likelihood BV parameters

        bc_bh = (BV_weights["Bc"].values[0], BV_weights["Bh"].values[0])

        print(f"Best BV parameters: {bc_bh}")

        return bc_bh


def process_weights_df(weights_df: pd.DataFrame, filter_contains: str = "train") -> np.ndarray:
    """
    Processes the weights DataFrame by filtering, dropping NaNs, and converting to a NumPy array.

    Parameters:
    - weights_df (pd.DataFrame): DataFrame containing at least 'weights' and 'calc_name' columns.
    - filter_contains (str): Substring that 'calc_name' should contain to be included.
    ...
    # Filter rows where 'calc_name' contains the specified substring
    filtered_df = weights_df[weights_df["calc_name"].str.contains(filter_contains, na=False)]

    # Drop rows with NaN in 'weights' column
    filtered_df = filtered_df.dropna(subset=["weights"])

    # Convert 'weights' column to a NumPy array of arrays
    weights_vals = np.array([np.array(w) for w in filtered_df["weights"].values])
