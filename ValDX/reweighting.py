...
from HDXer.reweighting_functions import (
    calc_trial_ave_lnpi,
    calc_trial_dfracs,
    calc_work,
    generate_trial_betas,
    read_contacts_hbonds,
    read_kints_segments,
...
class MaxEnt:
    ...
    def setup_no_runobj(self, folderlist, kint_file, expt_file_path, times):
        ...
        _contacts, _hbonds, _sorted_resids = read_contacts_hbonds(
            folderlist, self.runparams["contacts_prefix"], self.runparams["hbonds_prefix"]
        )
        if self.runparams["do_subsample"]:
            _contacts, _hbonds = subsample_contacts_hbonds(
                _contacts,
                _hbonds,
        ...
        _minuskt, _exp_dfrac, _segfilters = read_kints_segments(
            kint_file, expt_file_path, _nresidues, times, _sorted_resids
        )
        ...
        if self.methodparams["do_mcsampl"]:
            self.mcsamplvalues = {}
            mcsamplvalues = {
                "final_MClambdas": np.zeros(_nresidues),
        ...
        with open("%sinitial_params.dat" % self.runparams["out_prefix"], "w") as f:
            f.write(
                "Temp, kT, Convergence criterion, Beta_C, Beta_H, Gamma, Update rate (step size) factor, N frames\n"
            )
            f.write(
                "%s, %6.3f, %5.2e, %5.2f, %5.2f, %5.4e, %8.6f, %d\n"
                % (
        ...
        if self.runparams["iniweights"] is not None:
            try:
                ...
                raise HDX_Error(
                    "You supplied initial weights in a form that can't be converted to a numpy array, please check your inputs"
                )
            try:
                assert np.isclose(sum(self.runparams["iniweights"]), _nframes)
            except AssertionError:
                raise HDX_Error(
                    "You supplied initial weights but the weights don't add up to the number of trajectory frames"
                )
            try:
                assert len(self.runparams["iniweights"]) == _nframes
            except AssertionError:
                raise HDX_Error(
                    "You supplied initial weights but the number of weights doesn't equal the number of trajectory frames"
                )
            ...
            with open("%sinitial_params.dat" % self.runparams["out_prefix"], "a") as f:
                f.write(
                    "Initial weights were randomized, seed for np.random.RandomState = %d\n"
                    % statenum
                )
        ...
        with open("%swork.dat" % self.runparams["out_prefix"], "w") as f:
            f.write(
                "# gamma, weighted MSE to target, weighted RMSE to target, work (kJ/mol by default)\n"
            )
        with open("%sper_iteration_output.dat" % self.runparams["out_prefix"], "w") as f:
            f.write(
                "# Iteration, weighted MSE to target, weighted RMSE to target, Total lambdas, Fractional change in lambda, Curr. stepsize, Beta_C, Beta_H \n"
            )
        with open("%sper_restart_output.dat" % self.runparams["out_prefix"], "w") as f:
            f.write(
                "# Iteration, weighted MSE to target, weighted RMSE to target, Total lambdas, Fractional change in lambda, Curr. stepsize, Beta_C, Beta_H, Curr. work \n"
            )
    ...
    def setup_calc_hdx(self, resultsobj, analysisobj):
        ...
        _contacts, _hbonds, _sorted_resids = (
            resultsobj.contacts,
            resultsobj.hbonds,
            np.array([resultsobj.top.residue(residx).resSeq for residx in resultsobj.reslist]),
        )
        if self.runparams["do_subsample"]:
            _contacts, _hbonds = subsample_contacts_hbonds(
                _contacts,
        ...
        _kint = (
            np.repeat(_kint[:, np.newaxis], len(self.runparams["times"]), axis=1)
        ...
        for seg in _segments:
            ...
            _segfilters.append(
                np.in1d(_sorted_resids, seg_resids[1:])
            )  # Filter but skipping first residue in segment
        _segfilters = np.array(_segfilters)
        _segfilters = np.repeat(
            _segfilters[:, :, np.newaxis], len(self.runparams["times"]), axis=2
        )  # Repeat to shape (n_segments, n_residues, n_times)

        assert all(
            (
                _segfilters.shape == _exp_dfrac.shape,
                _segfilters.shape == _kint.shape,
                _nresidues == len(_sorted_resids),
            )
        )  # Check we've at least read in the right number!
        ...
        with open("%sinitial_params.dat" % self.runparams["out_prefix"], "w") as f:
            ...
            f.write(
                "%s, %6.3f, %5.2e, %5.2f, %5.2f, %5.2e, %8.6f, %d\n"
                % (
                    self.methodparams["temp"],
                    self.methodparams["kT"],
                    self.methodparams["tolerance"],
    ...
    def set_run_params(self, gamma, resultsobj, analysisobj, restart, paramdict):
        ...
        if resultsobj is not None:
            if analysisobj is not None:
                ...
                raise HDX_Error(
                    "You've supplied a BV results object from calc_hdx but not an Analysis object.\n"
                    "Both are required if you wish to continue a calc_hdx run."
                )
        if analysisobj is not None:
            raise HDX_Error(
                "You've supplied an Analysis object from calc_hdx but not a BV results object.\n"
                "Both are required if you wish to continue a calc_hdx run."
            )
    ...
    def update_lnpi_and_weights(self):
        ...
        if self.methodparams["do_mcsampl"]:
            biasfactor = np.sum(
                self.mcsamplvalues["lambdas_c"][:, np.newaxis] * self.runvalues["contacts"]
            ...
            biasfactor = np.sum(
                self.runvalues["lambdas"][:, np.newaxis] * self.runvalues["lnpi"], axis=0
            )  # Sum over all residues, = array of len(nframes). lambdas is 1D array broadcast to 2D
        ...
        self.runvalues["currweights"] = self.runvalues["currweights"] / np.sum(
            self.runvalues["currweights"]
        )
        self.runvalues["ave_lnpi"] = np.sum(
            self.runvalues["currweights"] * self.runvalues["lnpi"], axis=1
        )
        ...
        if self.runvalues["curriter"] == 0:
            _sigmalnpi = np.sum(
                self.runvalues["currweights"] * (self.runvalues["lnpi"] ** 2), axis=1
            )
        ...
        self.runvalues["ave_lnpi"] = np.repeat(
            self.runvalues["ave_lnpi"][:, np.newaxis], len(self.runparams["times"]), axis=1
        )
        self.runvalues["ave_lnpi"] = self.runvalues["ave_lnpi"][np.newaxis, :, :].repeat(
            self.runvalues["n_segs"], axis=0
        )
    ...
    def update_dfracs_and_mse(self):
        ...
        # D(t) = 1 - exp(-kt/P_i)
        self.runvalues["curr_residue_dfracs"] = 1.0 - np.exp(
            np.divide(
        ...
        self.runvalues["curr_segment_dfracs"] = _curr_segment_dfracs[:, np.newaxis, :].repeat(
            self.runvalues["segfilters"].shape[1], axis=1
        )
        ...
        self.runvalues["curr_MSE"] = (
            np.sum(
                (
                    self.runvalues["curr_segment_dfracs"] * self.runvalues["segfilters"]
    ...
    def update_work(self):
        ...
        self.runvalues["work"] = calc_work(
            self.runvalues["lnpi"],
            self.runvalues["lambdas"],
            self.runvalues["currweights"],
            self.methodparams["kT"],
        )
    ...
    def optimize_parameters_MC(self):
        ...
        for curr_mc_iter in range(self.methodparams["param_maxiters"]):
            ...
            trial_bv_bc, trial_bv_bh = generate_trial_betas(
                curr_bv_bc,
                curr_bv_bh,
            ...
            trial_ave_lnpi = calc_trial_ave_lnpi(
                _ave_contacts,
                _ave_hbonds,
                trial_bv_bc,
                trial_bv_bh,
                len(self.runparams["times"]),
                self.runvalues["n_segs"],
            )
            trial_residue_dfracs, trial_segment_dfracs, trial_MSE = calc_trial_dfracs(
                trial_ave_lnpi,
    ...
    def optimize_parameters_gradient(self):
        ...
        for curr_grad_iter in range(self.methodparams["param_maxiters"]):
            ...
            denom = self.runvalues["ave_lnpi"] * self.runvalues["segfilters"]
            rate_filtered = np.divide(
                -self.runvalues["minuskt_filtered"],
                np.exp(denom),
                out=np.full(self.runvalues["minuskt_filtered"].shape, np.nan),
                where=denom != 0,
            )
            der_dfrac_bh = np.nanmean(
                -(np.exp(-rate_filtered)) * rate_filtered * _ave_hbonds[np.newaxis, :, np.newaxis],
                axis=1,
            )  # need a sum over residues if data on fragments
            der_dfrac_bc = np.nanmean(
                -(np.exp(-rate_filtered))
                * rate_filtered
                * _ave_contacts[np.newaxis, :, np.newaxis],
                axis=1,
            )  # need a sum over residues if data on fragments
            secder_dfrac_bh = np.nanmean(
                _ave_hbonds[np.newaxis, :, np.newaxis] ** 2
                * (np.exp(-rate_filtered))
                * (rate_filtered - rate_filtered**2),
                axis=1,
            )
            secder_dfrac_bc = np.nanmean(
                _ave_contacts[np.newaxis, :, np.newaxis] ** 2
                * (np.exp(-rate_filtered))
                * (rate_filtered - rate_filtered**2),
                axis=1,
            )
            delta_byseg_dfrac = np.nanmean(
                (
                    np.where(
                        self.runvalues["segfilters"], self.runvalues["curr_segment_dfracs"], np.nan
                    )
                    - np.where(
                        self.runvalues["segfilters"], self.runvalues["exp_dfrac_filtered"], np.nan
                    )
                ),
                axis=1,
            )
            ...
            if (
                delta_bh / np.abs(self.methodparams["bv_bh"])
                > self.methodparams["param_stepfactor"]
            ):  # Scale steps if they're too large
                self.methodparams["bv_bh"] = np.abs(
                    self.methodparams["bv_bh"]
                    - self.methodparams["param_stepfactor"] * der_bh / np.abs(secder_bh)
                )
            else:
                self.methodparams["bv_bh"] = np.abs(
                    self.methodparams["bv_bh"] - der_bh / np.abs(secder_bh)
                )
            if (
                delta_bc / np.abs(self.methodparams["bv_bc"])
                > self.methodparams["param_stepfactor"]
            ):  # Scale steps if they're too large
                self.methodparams["bv_bc"] = np.abs(
                    self.methodparams["bv_bc"]
                    - self.methodparams["param_stepfactor"] * der_bc / np.abs(secder_bc)
                )
            else:
                self.methodparams["bv_bc"] = np.abs(
                    self.methodparams["bv_bc"] - der_bc / np.abs(secder_bc)
                )

            # Update values for the next iteration
            self.runvalues["ave_lnpi"] = calc_trial_ave_lnpi(
                _ave_contacts,
                _ave_hbonds,
                self.methodparams["bv_bc"],
                self.methodparams["bv_bh"],
            ...
            (
                self.runvalues["curr_residue_dfracs"],
                self.runvalues["curr_segment_dfracs"],
                self.runvalues["curr_MSE"],
            ) = calc_trial_dfracs(
                self.runvalues["ave_lnpi"],
                self.runvalues["segfilters"],
                self.runvalues["minuskt_filtered"],
                self.runvalues["exp_dfrac_filtered"],
                self.runvalues["n_datapoints"],
        ...
        # return currweights, bv bc and bv bh
    ...
    def sample_parameters_MC(self):
        ...
        def update_sampled_totals(
            total_bc, total_bh, total_mse, total_resfracs, total_lambdas_bc, total_lambdas_bh
        ):
        ...
        if self.methodparams["mc_equilsteps"] > 0:
            self.mcsamplvalues["smoothing_rate"] = self.methodparams["mc_equilsteps"] / np.sqrt(
                self.methodparams["mc_equilsteps"]
        ...
        for curr_mc_iter in range(self.methodparams["param_maxiters"]):
            ...
            if trial_MSE < self.runvalues["curr_MSE"]:
                ...
                trial_acc_val = (
                    ...
                    * len(self.runparams["times"])
                    * trial_MSE
                    / (2.0 * self.methodparams["mc_refvar"])
                )
                orig_acc_val = (
                    self.runvalues["n_segs"]
                    * len(self.runparams["times"])
            ...
            if self.methodparams["do_reweight"]:
                ...
                add_to_totals = update_sampled_totals(
                    bv_bc_sum,
                    bv_bh_sum,
                    MSE_sum,
                    residue_dfracs_sum,
                ...
                add_to_totals = update_sampled_totals_nolambda(
                    bv_bc_sum, bv_bh_sum, MSE_sum, residue_dfracs_sum
                )
                bv_bc_sum, bv_bh_sum, MSE_sum, residue_dfracs_sum = add_to_totals(
                    curr_bv_bc,
                    curr_bv_bh,
                    self.runvalues["curr_MSE"],
                    self.runvalues["curr_residue_dfracs"],
        ...
        if self.methodparams["do_reweight"]:
            (
                ...
                MSE_ave,
                residue_dfracs_ave,
                self.mcsamplvalues["ave_MClambdas_c"],
                self.mcsamplvalues["ave_MClambdas_h"],
            ) = update_sampled_averages(
                bv_bc_sum, bv_bh_sum, MSE_sum, residue_dfracs_sum, lambdas_bc_sum, lambdas_bh_sum
            )
        else:
            bv_bc_ave, bv_bh_ave, MSE_ave, residue_dfracs_ave = update_sampled_averages_nolambda(
                bv_bc_sum, bv_bh_sum, MSE_sum, residue_dfracs_sum
            )
        ...
        self.methodparams["bv_bh"] = (
            self.methodparams["bv_bh"] * (1.0 - self.mcsamplvalues["smoothing_rate"])
            + self.mcsamplvalues["smoothing_rate"] * bv_bh_ave
        )
        self.methodparams["bv_bc"] = (
            self.methodparams["bv_bc"] * (1.0 - self.mcsamplvalues["smoothing_rate"])
            + self.mcsamplvalues["smoothing_rate"] * bv_bc_ave
        )
        self.mcsamplvalues["MC_MSE_ave"] = (
            self.mcsamplvalues["MC_MSE_ave"] * (1.0 - self.mcsamplvalues["smoothing_rate"])
            + self.mcsamplvalues["smoothing_rate"] * MSE_ave
        )
        self.mcsamplvalues["MC_resfracs_ave"] = (
            self.mcsamplvalues["MC_resfracs_ave"] * (1.0 - self.mcsamplvalues["smoothing_rate"])
            + self.mcsamplvalues["smoothing_rate"] * residue_dfracs_ave
        )  # Not currently used but could be - average of residue fractions

        # Update lambdas using final Bh & Bc values if desired
        if self.methodparams["do_reweight"]:
            _lambdanewh = self.mcsamplvalues["final_MClambdas_h"] * (
                1.0 - self.mcsamplvalues["smoothing_rate"]
            ) + self.mcsamplvalues["smoothing_rate"] * (
                self.mcsamplvalues["ave_MClambdas_h"] / self.methodparams["param_maxiters"]
            )
            _lambdanewc = self.mcsamplvalues["final_MClambdas_c"] * (
                1.0 - self.mcsamplvalues["smoothing_rate"]
            ) + self.mcsamplvalues["smoothing_rate"] * (
                self.mcsamplvalues["ave_MClambdas_c"] / self.methodparams["param_maxiters"]
            )
            _lambdanew = 0.5 * (
                (_lambdanewh / self.methodparams["bv_bh"])
                + (_lambdanewc / self.methodparams["bv_bc"])
            )
            ...
            self.update_lambdas(
                self.mcsamplvalues["final_MClambdas"],
                self.mcsamplvalues["final_MClambdas_c"],
                self.mcsamplvalues["final_MClambdas_h"],
            )
    ...
    def calc_lambdas(self):
        ...
        curr_lambdas = np.nansum(
            ...
            / (np.sum(self.runvalues["segfilters"], axis=1)[:, 0])[:, np.newaxis],
            axis=0,
        )
        return curr_lambdas
    ...
    def update_lambdas(self, target_lambdas, target_lambdas_c=None, target_lambdas_h=None):
        ...
        if self.methodparams["do_mcsampl"]:
            ...
            self.mcsamplvalues["gamma_MClambdas"] = 0.5 * (
                (target_lambdas_h / self.methodparams["bv_bh"])
                + (target_lambdas_c / self.methodparams["bv_bc"])
            )

            # Calc example stepsize & make move in lambdas
            ave_deviation = np.sum(np.abs(target_lambdas)) / np.sum(
                self.mcsamplvalues["gamma_MClambdas"] != 0
            )
            self.runvalues["curr_lambda_stepsize"] = self.methodparams["stepfactor"] / (
                self.runparams["gamma"] * ave_deviation * self.runvalues["ave_sigma_lnpi"]
            )  # Example stepsize based on ave_sigma_lnpi
            self.mcsamplvalues["lambdas_c"] = self.mcsamplvalues["lambdas_c"] * (
                1.0 - self.runvalues["curr_lambda_stepsize"]
            ) + (self.runvalues["curr_lambda_stepsize"] * self.mcsamplvalues["gamma_MClambdas_c"])
            self.mcsamplvalues["lambdas_h"] = self.mcsamplvalues["lambdas_h"] * (
                1.0 - self.runvalues["curr_lambda_stepsize"]
            ) + (self.runvalues["curr_lambda_stepsize"] * self.mcsamplvalues["gamma_MClambdas_h"])
            self.runvalues["lambdas"] = self.runvalues["lambdas"] * (
                1.0 - self.runvalues["curr_lambda_stepsize"]
    ...
    def make_iteration(self):
        ...
        # Update lambdas
        if not self.methodparams[
            "do_mcsampl"
        ]:  # Lambdas are updated internally in the sampling function if needed
            if self.methodparams["do_reweight"]:
        ...
        if not self.methodparams["do_reweight"]:
            ...
            self.runvalues["delta_lambda_mod"] = (
                self.runvalues["lambda_mod"] - _prev_lambda_mod
            ) / self.runvalues["lambda_mod"]

            if self.runvalues["curriter"] >= 100:
                if (
                    self.runvalues["delta_lambda_mod"] * _prev_delta_lambda_mod < 0
                ):  # if oscillations in sign of delta_lambda
                    self.methodparams["stepfactor"] = (
                        self.methodparams["stepfactor"] / self.methodparams["stepfactor_scaling"]
                    )
    ...
    def write_iteration(self):
        ...
        with open("%sper_iteration_output.dat" % self.runparams["out_prefix"], "a") as f:
            f.write(
                "%6d %8.6f %8.6f %10.8e %10.8e %10.8e %10.8e %10.8e \n"
                % (
    ...
    def write_restart(self):
        ...
        with open("%sper_restart_output.dat" % self.runparams["out_prefix"], "a") as f:
            f.write(
                "%6d %8.6f %8.6f %10.8e %10.8e %10.8e %10.8e %10.8e %8.5f \n"
                % (
                    self.runvalues["curriter"],
                    self.runvalues["curr_MSE"],
                    np.sqrt(self.runvalues["curr_MSE"]),
    ...
    def run(self, gamma=10**-2, resultsobj=None, analysisobj=None, restart=None, **run_params):
        ...
        if self.runparams["from_restart"]:
            ...
            try:
                self.setup_no_runobj(
                    self.runparams["data_folders"],
                ...
                raise HDX_Error(
                    "Missing parameters to set up a reweighting run.\n"
        ...
        try:
            ...
            if self.methodparams["maxiters"] > 1000:
                self.runparams["restart_interval"] = int(self.methodparams["maxiters"] / 1000)
            else:
                self.runparams["restart_interval"] = self.methodparams["maxiters"]

        # Start timer for iterations
        start_time = time.time()

        # Do iterations until either maxiters reached or convergence
        while (
            self.runvalues["curriter"] <= self.methodparams["maxiters"]
            and not self.runvalues["is_converged"]
        ):
            self.make_iteration()
        ...
        # Stop timer and compute average speed
        end_time = time.time()
        total_iters = self.runvalues["curriter"]
        elapsed = end_time - start_time
        avg_speed = total_iters / elapsed if elapsed > 0 else float("inf")

        # Print and save average iteration speed
        print(f"Average iteration speed: {avg_speed:.2f} iterations/sec")
        speed_file = f"{self.runparams['out_prefix']}iteration_speed.txt"
        with open(speed_file, "w") as f:
            f.write(f"Average iteration speed: {avg_speed:.2f} iterations/sec\n")

        # 3) Do final save/cleanup
        np.savetxt(
            f"{self.runparams['out_prefix']}final_segment_fractions.dat",
            np.sum(self.runvalues["curr_segment_dfracs"] * self.runvalues["segfilters"], axis=1)
            / np.sum(self.runvalues["segfilters"], axis=1),
            header="Times: " + " ".join(str(t) for t in self.runparams["times"]),
            fmt="%8.5f",
        )
        np.savetxt(
            f"{self.runparams['out_prefix']}final_weights.dat", self.runvalues["currweights"]
        )

        with open(f"{self.runparams['out_prefix']}work.dat", "a") as f:
            f.write(
                "%5.4e %8.6f %8.6f %8.5f \n"
                % (
                    self.runparams["gamma"],
        ...
        with open(f"{self.runparams['out_prefix']}per_iteration_output.dat", "a") as f:
            f.write(
                "%6d %8.6f %8.6f %10.8e %10.8e %10.8e %10.8e %10.8e # FINAL values at convergence or max iterations\n"
                % (
                    self.runvalues["curriter"],
                    self.runvalues["curr_MSE"],
                    np.sqrt(self.runvalues["curr_MSE"]),
