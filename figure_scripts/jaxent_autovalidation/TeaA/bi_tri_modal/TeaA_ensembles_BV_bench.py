...
def pdb_to_2frame_xtc(pdb_path, xtc_path=None):
    """
    Creates a 2-frame XTC trajectory from a PDB file,
    where both frames are identical to the PDB structure.
    """
    if xtc_path is None:
        xtc_path = pdb_path.replace(".pdb", "_2frame.xtc")
    ...
    try:
        ...
        with XTCWriter(xtc_path, n_atoms=u.atoms.n_atoms) as W:
            ...
            W.write(u.atoms)
            W.write(u.atoms)
        print(f"Created 2-frame XTC: {xtc_path}")
        return xtc_path
...
def pre_process_main_BPTI():
    ...
    test_names = ["TeaA_ISO_bi", "TeaA_ISO_tri", "TeaA_ISO_open", "TeaA_ISO_closed"]
    ...
    open_reference_path = "/home/alexi/Documents/ValDX/figure_scripts/jaxent_autovalidation/_Bradshaw/Reproducibility_pack_v2/data/trajectories/TeaA_ref_open_state.pdb"
    closed_reference_path = "/home/alexi/Documents/ValDX/figure_scripts/jaxent_autovalidation/_Bradshaw/Reproducibility_pack_v2/data/trajectories/TeaA_ref_closed_state.pdb"

    # Generate 2-frame XTC files from reference PDBs
    open_reference_xtc_path = pdb_to_2frame_xtc(open_reference_path)
    closed_reference_xtc_path = pdb_to_2frame_xtc(closed_reference_path)

    # You might want to use open_reference_xtc_path and closed_reference_xtc_path later
    # For now, just printing them to confirm creation
    print(f"Open reference XTC: {open_reference_xtc_path}")
    print(f"Closed reference XTC: {closed_reference_xtc_path}")

    top_paths = [
        topology_path,
        topology_path,
        open_reference_path,
        closed_reference_path,
        # dirty_top_path,
        # clean_top_path,
        # shaw_top_path,
        # badMD_top_path,
    ...
    traj_paths = [
        bi_trajectory_path,
        tri_trajectory_path,
        open_reference_xtc_path,
        closed_reference_xtc_path,
        # dirty_traj_paths[0],
        # clean_traj_paths[0],
        # shaw_traj_paths[0],
        # badMD_traj_path[0],
