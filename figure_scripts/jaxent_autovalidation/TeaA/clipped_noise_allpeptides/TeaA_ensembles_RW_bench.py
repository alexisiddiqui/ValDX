...
def pre_process_main_BPTI():
    ...
    trajectory_path = "/home/alexi/Documents/ValDX/figure_scripts/jaxent_autovalidation/_TeaA/trajectories/TeaA_filtered.xtc"
...
# %%
base_hdx_path = "/home/alexi/Documents/ValDX/figure_scripts/jaxent_autovalidation/TeaA/clipped_noise_allpeptides/noisy_dfrac_sd_"
suffix = ".dat"


base_dir = os.path.dirname(base_hdx_path)
# %%
times = [0.167, 1, 10, 60, 120]

noises = [0, 0.01, 0.05, 0.1, 0.2, 0.5, 1.0]

for idx, (test_name, top_path, traj_paths) in enumerate(
    zip(test_names[:1], top_paths[:1], traj_paths[:1])
):
    for noise in noises:
        hdx_path = base_hdx_path + str(noise) + suffix

        noise_dir = os.path.join(
            base_dir,
            f"noisy_dfrac_sd_{noise}",
        )

        os.makedirs(noise_dir, exist_ok=True)
        # change the hdx_path to the new directory
        os.chdir(noise_dir)
