...
def pre_process_main_BPTI():
    ...
    # Build lists by scanning the JAX-ENT trajectories directory for sliced_* subfolders.
    # Each sliced_N folder is expected to contain:
    #   - TeaA_filtered_sliced.xtc  (bi trajectory)
    #   - TeaA_initial_sliced.xtc   (tri trajectory)
    base_traj_dir = "/home/alexi/Documents/JAX-ENT/jaxent/examples/1_IsoValidation_OMass/data/_Bradshaw/Reproducibility_pack_v2/data/trajectories"
    
    slices = [1,2,5,10,20,50,100,1000]
    slices = [20,50,100,1000]

    sliced_dirs = [
        d for d in os.listdir(base_traj_dir)
        if d.startswith("sliced_") and os.path.isdir(os.path.join(base_traj_dir, d))
    ]
    # sort numerically by the suffix after "sliced_"
    def sliced_key(name):
        try:
    ...
    sliced_dirs = sorted(sliced_dirs, key=sliced_key)

    # Filter to only include the requested slices (from `slices` list)
    # allow slices list to contain ints or strings
    # build allowed set from the requested slices
    allowed = {int(s) for s in slices}

    # Safely extract the numeric suffix from names like "sliced_100" or "sliced_100_extra"
    filtered = []
    for d in sliced_dirs:
        # ensure there is a suffix after the first underscore
        parts = d.split("_", 1)
        if len(parts) < 2:
            continue
        # take the first token after "sliced_"; this handles "sliced_100_extra"
        suffix_token = parts[1].split("_", 1)[0]
        try:
            n = int(suffix_token)
        except ValueError:
            # non-numeric suffix (e.g. "trajectories") — skip
            continue
        if n in allowed:
            filtered.append(d)

    sliced_dirs = filtered
    ...
    for d in sliced_dirs:
        n = d.split("_", 1)[1]
        dir_path = os.path.join(base_traj_dir, d)
        bi_path = os.path.join(dir_path, "TeaA_filtered_sliced.xtc")
        tri_path = os.path.join(dir_path, "TeaA_initial_sliced.xtc")

        # Only add entries if the expected files exist
        if os.path.exists(bi_path):
            top_paths.append(topology_path)
            traj_paths.append(bi_path)
            test_names.append(f"TeaA_ISO_bi_sliced_{n}")
        else:
            print(f"Warning: missing {bi_path} — skipping bi for {d}")

        if os.path.exists(tri_path):
            top_paths.append(topology_path)
            traj_paths.append(tri_path)
            test_names.append(f"TeaA_ISO_tri_sliced_{n}")
        else:
            print(f"Warning: missing {tri_path} — skipping tri for {d}")
...
# CSV path for saving per-experiment timings
times_csv_path = os.path.join(os.getcwd(), "experiment_times.csv")
...
def append_time_record(csv_path, record):
    """Append a dict record to csv_path. Write header if file doesn't exist."""
...
for idx, (test_name, top_path, traj_paths) in enumerate(zip(test_names, top_paths, traj_paths)):
    ...
    # compute and save iteration timing
    iter_end = time.time()
    iter_duration_s = iter_end - iter_start
    iter_duration_min = iter_duration_s / 60.0

    # record to CSV
    time_record = {
        "test_name": test_name,
        "duration_seconds": f"{iter_duration_s:.6f}",
        "duration_minutes": f"{iter_duration_min:.6f}",
        "completed_at": time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(iter_end)),
    }
    append_time_record(times_csv_path, time_record)

    # also print timing
    end_time = time.time()

    print(f"Time taken for {test_name}: {iter_duration_s:.2f} s")
    print(f"Time taken for {test_name}: {iter_duration_min:.2f} min")

    # time in minutes for whole run (unchanged behaviour)
    print(f"Total elapsed since script start: {(end_time - start_time) / 60} min")
