...
def parse():
    ...
    parser.add_argument("-nopm","--no_params", \
                        ...
                        default=True, dest='do_params', action='store_false')
    ...
    parser.add_argument("-rst", "--restart", help="Name of the pkl restart file with parameters for continuing reweighting.", \
                        type=str)
...
# (Only applicable if we add extra lines here to read in the extra Contacts/Hbonds files for each chain - currently his is specific for TeaA)
...
#    if mc_sample_params:
    ...
    # Random initial weights perturb the ensemble - e.g. can we recreate the original trajectory with equally weighted frames?
...
for ncount in range(currcount+1, count+1):
    ...
    if mc_sample_params:
       ...
       for iterpar in range(0, paramcount):
           ...
           like = normseg * chisquarenew / (2.0*chisquareref)
           likeold = normseg * chisquare / (2.0*chisquareref)
           if Bhnew > 0:
              if Bcnew > 0:
    ...
    if update_lambdas:
       ...
       rate = ratef / (gamma * avehdxdev * avesigmalnpi) # Step size calc
    ...
    # Uncomment to break out at 1st iteration
#    break
