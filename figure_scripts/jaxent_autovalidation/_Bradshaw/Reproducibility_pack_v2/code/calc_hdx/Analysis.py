...
plt.rc(
    "axes",
    prop_cycle=(
        cycler("color", ["k", "b", "r", "orange", "c", "m", "y", "g"])
    ),  # Color cycle defaults to black
    labelweight="heavy",
    labelsize=14,
    titlesize=18,
)  # Default fontsizes for printing
...
class Analyze:
    ...
    def __init__(self, resobj, top, **extra_params):
        ...
        try:
            ...
            self.resfracs = np.reshape(
                resobj.resfracs[:, :, 0], (1, len(resobj.resfracs), len(resobj.resfracs[0]))
            )
            ...
            raise Functions.HDX_Error(
                "Error when copying results from prediction to analysis objects - have you made any HDX predictions yet?"
            )
        ...
        try:
            ...
            print(
                "Couldn't load extra parameters for analysis (maybe they weren't provided?).\nUsing previous parameters from %s object."
                % resobj
            )
    ...
    def __add__(self, other):
        ...
        if isinstance(other, Analyze):
            ...
            if not all(
                (
                    np.array_equal(self.residxs, other.residxs),
                    np.array_equal(self.rates, other.rates),
                )
            ):
            ...
            if type(self.resobj) is Methods.Radou:
                ...
                new.c_lnpfs = np.append(
                    new.c_lnpfs, np.mean(new.lnpf_byframe, axis=1)[np.newaxis, :], axis=0
                )
            ...
            if type(self.resobj) is Methods.Radou:
                for i2, t in enumerate(new.params["times"]):
                    ...
                    for i1, curr_frac in enumerate(
                        map(_residue_fraction_lnpf, new.c_lnpfs[-1], new.rates)
                    ):
                        _[i1, i2] = curr_frac
            else:
                for i2, t in enumerate(new.params["times"]):
                    ...
                    for i1, curr_frac in enumerate(
                        map(_residue_fraction, new.c_pfs[-1], new.rates)
                    ):
                        _[i1, i2] = curr_frac
            new.c_resfracs = np.concatenate(
                (new.c_resfracs, np.reshape(_, (1, len(new.residxs), len(new.params["times"])))),
                axis=0,
            )
    ...
    def __setstate__(self, d):
        ...
        if os.path.exists(self.params["outprefix"] + "topology.pkl"):
            try:
                ...
                raise Functions.HDX_Error(
                    "Can't read cached topology file %s. "
    ...
    def _cumulative_average(self, data, blocksizes):
        ...
        if not len(data) == np.sum(blocksizes):
            raise Functions.HDX_Error(
                "Unable to cumulatively average data of length %d using total blocksizes %d"
    ...
    def read_segfile(self):
        ...
        try:
            # Attempt to read 3 columns first
            self.segres = np.loadtxt(
                self.params["segfile"], dtype=[("segres", np.int32, (2,)), ("chain", np.int32, (1))]
            )  # ResIDs will be converted to indices with dictionary in segments function
            with open(self.params["logfile"], "a") as f:
                f.write(
                    "Chain indices read from segments file - segment averaging will be performed on defined chains\n"
                )
            self._single_chain = False
        except ValueError as e:
            # Check if the ValueError is due to column mismatch (specifically 2 columns found when 3 expected)
            error_str = str(e).lower()
            # Check for common numpy error messages related to column count mismatch
            is_col_mismatch = (
                "wrong number of columns" in error_str
                or "number of columns changed" in error_str
                or (
                    "lines were found with different numbers of columns" in error_str
                    and "expected 3" in error_str
                )
            )  # More specific check

            if is_col_mismatch:
                # If it seems like a column mismatch, try reading 2 columns
                try:
                    # Explicitly use only the first two columns
                    tmp_segres = np.loadtxt(self.params["segfile"], dtype=np.int32, usecols=(0, 1))
                    with open(self.params["logfile"], "a") as f:
                        f.write(
                            "Chain indices NOT read from segments file (expected 3 columns, found 2). Assuming single chain (index 0).\n"
                        )
                    # Initialize the structured array correctly
                    self.segres = np.zeros(
                        len(tmp_segres),
                        dtype=[("segres", np.int32, (2,)), ("chain", np.int32, (1))],
                    )
                    self.segres["segres"] = tmp_segres
                    self.segres["chain"] = 0  # Default to chain 0
                    self._single_chain = True
                except (
                    ValueError
                ) as e2:  # Catch potential errors during the 2-column read (e.g., non-integer)
                    raise Functions.HDX_Error(
                        "Attempted to read segments file '%s' as 2-column format after 3-column read failed, but encountered another error: %s\n"
                        "File should contain 2 columns of integers, separated by spaces.\n"
                        "Format: start_residue end_residue" % (self.params["segfile"], str(e2))
                    )
                except (
                    IndexError
                ) as e2:  # Catch potential errors like empty file during 2-column read
                    raise Functions.HDX_Error(
                        "Attempted to read segments file '%s' as 2-column format after 3-column read failed, but encountered an index error (possibly empty file?): %s\n"
                        "File should contain 2 columns of integers, separated by spaces.\n"
                        "Format: start_residue end_residue" % (self.params["segfile"], str(e2))
                    )
                except Exception as e2:  # Catch any other unexpected error during 2-column read
                    raise Functions.HDX_Error(
                        "An unexpected error occurred while attempting to read segments file '%s' as 2-column format: %s\n"
                        % (self.params["segfile"], str(e2))
                    )
            else:
                # If it's a different ValueError (e.g., non-numeric data in the 3-column attempt), raise the specific error
                raise Functions.HDX_Error(
                    "There's a problem reading the values in your segments file: %s \n"
                    "This might be due to non-integer data when expecting 3 columns, or another format issue.\n"
                    "Original Error: %s\n"
                    "Expected format: start_residue end_residue chain_index[optional]"
                    % (self.params["segfile"], str(e))
                )
        # Note: FileNotFoundError will propagate naturally if the file doesn't exist.
    ...
    def read_expfile(self):
        ...
        try:
            ...
            raise Functions.HDX_Error(
                "There's a problem with the experimental data file. It has too few timepoints. \n"
        ...
        try:
            ...
            raise Functions.HDX_Error(
                "There's a problem with the experimental data file. It has too many timepoints. \n"
        ...
        if self._single_chain:
            if np.array_equal(self.segres["segres"], expt["segres"]):
                ...
                raise Functions.HDX_Error(
                    "The experimental segments read from %s and predicted segments read from %s don't match!"
                    % (self.params["segfile"], self.params["expfile"])
                )
        else:
            if all(
                (
                    np.array_equal(self.segres["segres"], expt["segres"]),
                    np.array_equal(self.segres["chain"], expt["chain"]),
                )
            ):
                self.expfracs = expt["fracs"]
            else:
                raise Functions.HDX_Error(
                    "The experimental segments/chains read from %s and predicted segments/chains read from %s don't match!"
                    % (self.params["segfile"], self.params["expfile"])
                )
        ...
        if self.params["skip_first"]:
            for i1, (seg, chain) in enumerate(self.segres):
                with open(self.params["logfile"], "a") as f:
                    try:
                        f.write(
                            "'Skip_first' is set. Not including residue %s in averaging for segment %s-%s, chain idx %s.\n"
            ...
            for i1, (seg, chain) in enumerate(self.segres):
                with open(self.params["logfile"], "a") as f:
                    try:
                        f.write(
                            "'Skip_first' is NOT set. Including residue %s in averaging for segment %s-%s, chain idx %s.\n"
                            % (top.residue(res2idx[(seg[0], chain)]), seg[0], seg[1], chain)
                        )
                    except KeyError:
                        _ = top.chain(chain).residue(0)
        ...
        for i0, chunk, errchunk in zip(range(len(self.resfracs)), self.resfracs, self.resfrac_STDs):
            for i2, t in enumerate(self.params["times"]):
                for i1, (seg, chain) in enumerate(self.segres):
                    ...
                    if self.params["skip_first"]:
                        idxs = np.where(np.logical_and(self.residxs > start, self.residxs <= end))[
                            0
                        ]  # > start skips
                    else:
                        idxs = np.where(np.logical_and(self.residxs >= start, self.residxs <= end))[
                    ...
                    stddevs[i0, i1, i2] = np.sqrt(np.sum(errchunk[idxs, i2] ** 2)) / len(
                        np.nonzero(idxs)
                    )
        ...
        if self._single_chain:
            for chunkave in aves:
                if os.path.exists(self.params["outprefix"] + "Segment_average_fractions.dat"):
                    filenum = len(
                        glob.glob(self.params["outprefix"] + "Segment_average_fractions*")
                    )
                    np.savetxt(
                        self.params["outprefix"]
                        + "Segment_average_fractions_chunk_%d.dat" % (filenum + 1),
                        np.hstack((self.segres["segres"], chunkave)),
                        fmt="%6d %6d " + "%8.5f " * len(self.params["times"]),
                        header="Res1   Res2  Times / min: %s"
                        % " ".join([str(t) for t in self.params["times"]]),
                    )
                else:
                    np.savetxt(
                        self.params["outprefix"] + "Segment_average_fractions.dat",
                        np.hstack((self.segres["segres"], chunkave)),
                        fmt="%6d %6d " + "%8.5f " * len(self.params["times"]),
                        header="Res1   Res2  Times / min: %s"
                        % " ".join([str(t) for t in self.params["times"]]),
                ...
                if os.path.exists(self.params["outprefix"] + "Segment_average_fractions.dat"):
                    ...
                    np.savetxt(
                        ...
                        np.hstack(
                            (
                                self.segres["segres"],
                                self.segres["chain"].reshape((len(self.segres["segres"]), 1)),
                                chunkave,
                            )
                        ),
                        fmt="%6d %6d %6d " + "%8.5f " * len(self.params["times"]),
                        header="Res1   Res2  Chain  Times / min: %s"
                        % " ".join([str(t) for t in self.params["times"]]),
                    )
                else:
                    np.savetxt(
    ...
    def check_blocksize(self):
        ...
        with np.errstate(invalid="ignore"):  # Ignores infs in stderr calc
            for i, window in enumerate(valid_windows):
                ...
                self.tot_SEMs[i, 1] = stderr(
                    self._windowed_average(np.sum(self.pf_byframe, axis=0), window)
                )
        ...
        with np.errstate(invalid="ignore"):  # Ignores infs in stderr calc
            for j, res in enumerate(self.resnums):
                for i, window in enumerate(valid_windows):
                    ...
                    self.res_SEMs[j, i, 1] = stderr(
                        self._windowed_average(self.pf_byframe[j], window)
                    )
                    self.res_STDs[j, i, 1] = np.std(
                        self._windowed_average(self.pf_byframe[j], window), ddof=1
                    )
    ...
    def propagate_errors(self):
        ...
        for i in range(self.pfs.shape[0]):
            self.resfrac_STDs[i] = self.resobj.dfrac(
                write=False,
                use_self=False,
                alternate_pfs=np.stack((self.pfs[i], self.pf_stds[i]), axis=1),
            )[:, :, 1]
    ...
    def desc_stats(self):
        ...
        with open(self.params["outprefix"] + "Descriptive_statistics.dat", "a") as f:
            np.savetxt(
                f,
                self.correls,
                header="Pearson's R correlation, Times / min: %s"
                % " ".join([str(t) for t in self.params["times"]]),
                fmt="%8.6f",
            )
            np.savetxt(
                f,
                self.MSE,
                header="Mean signed error / frac, Pred. - Expt., Times / min: %s"
                % " ".join([str(t) for t in self.params["times"]]),
                fmt="%10.8f",
            )
            np.savetxt(
                f,
                self.MUE,
                header="Mean unsigned error / frac, Pred. - Expt., Times / min: %s"
                % " ".join([str(t) for t in self.params["times"]]),
                fmt="%10.8f",
            )
    ...
    def print_summaries(self):
        ...
        try:
            if os.path.exists(self.params["outprefix"] + "SUMMARY_protection_factors.dat"):
                ...
                np.savetxt(
                    self.params["outprefix"] + "SUMMARY_protection_factors_%d.dat" % (filenum + 1),
                    np.stack((self.resnums, self.c_pfs[-1]), axis=1),
                    fmt=["%7d", "%18.8f"],
                    header="ResID  Protection factor",
                )  # Use residue indices internally, print out IDs
            else:
                np.savetxt(
                    self.params["outprefix"] + "SUMMARY_protection_factors.dat",
                    np.stack((self.resnums, self.c_pfs[-1]), axis=1),
                    fmt=["%7d", "%18.8f"],
                    header="ResID  Protection factor",
                )  # Use residue indices internally, print out IDs
            ...
            raise Functions.HDX_Error(
                "Can't write summary protection factors - perhaps you haven't calculated them yet?"
            )
        ...
        try:
            if os.path.exists(self.params["outprefix"] + "SUMMARY_logProtection_factors.dat"):
                filenum = len(
                    glob.glob(self.params["outprefix"] + "SUMMARY_logProtection_factors*")
                )
                np.savetxt(
                    self.params["outprefix"]
                    + "SUMMARY_logProtection_factors_%d.dat" % (filenum + 1),
                    np.stack((self.resnums, self.c_lnpfs[-1]), axis=1),
                    fmt=["%7d", "%18.8f"],
                    header="ResID  ln(Protection factor)",
                )  # Use residue indices internally, print out IDs
            else:
                np.savetxt(
                    self.params["outprefix"] + "SUMMARY_logProtection_factors.dat",
                    np.stack((self.resnums, self.c_lnpfs[-1]), axis=1),
                    fmt=["%7d", "%18.8f"],
                    header="ResID  ln(Protection factor)",
                )  # Use residue indices internally, print out IDs
            ...
            if type(self.resobj) is Methods.Radou:
                raise Functions.HDX_Error(
                    "Can't write summary log protection factors - perhaps you haven't calculated them yet?"
                )
            else:
                pass
        ...
        try:
            if os.path.exists(self.params["outprefix"] + "SUMMARY_residue_fractions.dat"):
                ...
                np.savetxt(
                    self.params["outprefix"] + "SUMMARY_residue_fractions_%d.dat" % (filenum + 1),
                    np.concatenate(
                        (np.reshape(self.resnums, (len(self.residxs), 1)), self.c_resfracs[-1]),
                        axis=1,
                    ),
                ...
                np.savetxt(
                    self.params["outprefix"] + "SUMMARY_residue_fractions.dat",
                    np.concatenate(
                        (np.reshape(self.resnums, (len(self.residxs), 1)), self.c_resfracs[-1]),
                        axis=1,
                    ),
            ...
            raise Functions.HDX_Error(
                "Can't write summary residue fractions - perhaps you haven't calculated them yet?"
            )
        ...
        try:
            if self._single_chain:
                if os.path.exists(
                    self.params["outprefix"] + "SUMMARY_segment_average_fractions.dat"
                ):
                    filenum = len(
                        glob.glob(self.params["outprefix"] + "SUMMARY_segment_average_fractions*")
                    )
                    np.savetxt(
                        self.params["outprefix"]
                        + "SUMMARY_segment_average_fractions_%d.dat" % (filenum + 1),
            ...
            raise Functions.HDX_Error(
                "Can't write summary segment fractions - perhaps they haven't been calculated yet?"
            )
...
class MultiAnalyze:
    ...
    def __init__(self, filenames):
        ...
        summed_objs = sum(
            objects[1:], objects[0]
        )  # Checks for object type & equivalence of rates/residxs
        to_average = [""]
...
class Plots:
    ...
    def __init__(self, aobj):
        ...
        if isinstance(aobj, Analyze):
            ...
            raise Functions.HDX_Error(
                "Can't initialize a Plots object from anything"
    ...
    def choose_plots(self, **override_opts):
        ...
        try:
            ...
            self.avail["_expt_overlay"] = len(self.results.c_segfracs[-1]) == len(
        ...
        if override_opts == None:
            ...
            try:
                ...
                with open(self.results.params["logfile"], "a") as f:
                    f.write(
                        "Available plots manually overriden for plots: %s \n"
    ...
    def df_curve(self):
        ...
        def _plot_df_curve(ax, segdata, blockdata, overlay_ys=None, **plot_opts):
            ...
            ax.fill_between(
                ...
                color="gray",
                alpha=0.3,
                label="Std. Dev. across trajectory blocks",
            )
        ...
        def _plot_pdf_pages_df(startslice, endslice):
            ...
            fig1, axs1 = plt.subplots(
                ncols=4, nrows=3, sharex=True, sharey=True, figsize=(11, 8.5)
            )  # Letter
            ...
            if self.avail["_expt_overlay"]:
                for a, predsegs, sliceidx, expt in zip(
                    axs1,
                    np.hstack((self.results.segres["segres"], self.results.c_segfracs[-1]))[
            ...
            if self.avail["_expt_overlay"]:
                for a, predsegs, sliceidx, expt in zip(
                    ...
                    _plot_log_df_curve(
                        a, predsegs, self.results.segfracs[:, sliceidx], overlay_ys=expt
                    )
            else:
                for a, predsegs, sliceidx in zip(
                    axs2,
                    np.hstack((self.results.segres["segres"], self.results.c_segfracs[-1]))[
                        startslice : endslice + 1
                    ],
        ...
        with PdfPages(self.results.params["outprefix"] + "df_curves.pdf") as pdf:
            ...
            try:
                ...
                currfig, logfig = _plot_pdf_pages_df(
                    12 * (pg - 1), len(self.results.c_segfracs[-1])
                )
                logfigs.append(logfig)
                pdf.savefig(currfig)
                plt.close()
    ...
    def df_convergence(self):
        ...
        def _plot_df_convergence(block_fracs, cumul_fracs, block_errs, seg):
            ...
            for timeidx in range(len(self.results.params["times"])):
                l = ax.plot(
                    xs,
                    cumul_fracs[:, timeidx],
                    label="Time %s min, blocks +/- std. err."
                    % self.results.params["times"][timeidx],
                )
                ax.errorbar(
                    xs,
                    block_fracs[:, timeidx],
                    yerr=block_errs[:, timeidx],
                    fmt="o",
                    capsize=2,
                    color=l[-1].get_color(),
                )
        ...
        with PdfPages(self.results.params["outprefix"] + "df_convergence.pdf") as pdf:
            for i, currseg in enumerate(self.results.segres["segres"]):
                currfig = _plot_df_convergence(
                    self.results.segfracs[:, i, :],
    ...
    def seg_curve(self):
        ...
        def _plot_seg_curve(ax, cumul_fracs, seglist, blocksize, time, overlay_fracs=None):
            ...
            if overlay_fracs is not None:
                ...
                ax.plot(xs, overlay_fracs, label="Experimental fraction", linestyle=":")
            ...
            for timeidx, t in enumerate(self.results.params["times"]):
                ...
                if self.avail["_expt_overlay"]:
                    ...
                    ax1 = _plot_seg_curve(
                        ...
                        self.results.segres["segres"],
                        self.results.n_frames[0],
                        t,
                        self.results.expfracs[:, timeidx],
                    )
                    # +/- std.dev
                    ax1 = _fill_seg_range(
                        ax1,
                        self.results.segfracs[:, :, timeidx],
                        self.results.segres["segres"],
                        self.results.c_segfracs[-1, :, timeidx],
                    )
                else:
    ...
    def tot_pf(self):
        ...
        with PdfPages(self.results.params["outprefix"] + "tot_pf.pdf") as pdf:
            ...
            ax.errorbar(
                xs,
                np.sum(self.results.pfs, axis=1),
                yerr=tots,
                label="Block protection factor +/- std. err.",
                fmt="o",
                capsize=2,
                color=l[-1].get_color(),
            )
            ...
            ax.set_ylim(
                _get_ylim(
                    np.min(np.sum(self.results.pfs, axis=1)),
                    np.max(np.sum(self.results.pfs, axis=1)),
                )
            )
            ax.legend()
            fig.tight_layout()  # No fig.suptitle = default figure coords
            ...
            ax.set_yscale("log")
            ax.set_ylim(
                10 ** np.floor(np.log10(np.max(np.sum(self.results.pfs, axis=1)))),
    ...
    def pf_error(self):
        ...
        with PdfPages(self.results.params["outprefix"] + "pf_error.pdf") as pdf:
            ...
            ax.plot(
                xs,
                ys,
                label="Individual residue PF normalized std. errors",
                linewidth=1,
                alpha=0.2,
                color="gray",
            )
