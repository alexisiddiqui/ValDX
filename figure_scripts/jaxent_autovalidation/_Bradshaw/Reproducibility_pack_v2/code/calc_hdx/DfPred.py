...
class DfPredictor(object):
    ...
    def __getstate__(self):
        ...
        if os.path.exists(self.params["outprefix"] + "topology.pkl"):
            ...
            pickle.dump(
                odict.pop("top"), open(self.params["outprefix"] + "topology.pkl", "wb"), protocol=-1
            )
    ...
    def __setstate__(self, d):
        ...
        if os.path.exists(self.params["outprefix"] + "topology.pkl"):
            ...
            raise Functions.HDX_Error(
                "No such cache file %s. Re-run the calculation, it should be "
    ...
    def assign_cis_proline(self):
        ...
        if prolines is None:
            ...
            for i, proidx in enumerate(prolines[:, 1]):
                self.top.residue(proidx).cis_byframe = np.logical_and(
                    outangs < np.pi / 2, outangs > -1 * np.pi / 2
                )[:, i]
                if np.max(self.top.residue(proidx).cis_byframe) > 0:
                    with open(self.params["logfile"], "a") as f:
                        f.write(
                            "Cis-proline found at frame %d for residue %s!\n"
                            % (
                                np.argmax(self.top.residue(proidx).cis_byframe) + 1,
                                self.top.residue(proidx).resSeq,
                            )
                        )
    ...
    def assign_disulfide(self):
        ...
        cyx = [res for res in self.top.residues if res.name == "CYS" and isCyx(res)]
    ...
    def assign_his_protonation(self):
        ...
        for i in hisidx:
            ...
            if all(
                n in names for n in ["HD1", "HE2"]
            ):  # Atom names for doubly protonated His (Hip)
        ...
        for c in self.top.chains:
            ...
            with open(self.params["logfile"], "a") as f:
                if any((nterm_manual, cterm_manual)):
                    f.write(
                        "One or more of the chain termini is not a protein residue.\n"
    ...
    def _adj_to_rates(self, rate_adjs):
        ...
        lgkAexp = self.params["kint_params"]["lgkAref"] - (
            self.params["kint_params"]["EaA"] / np.log(10) / self.params["kint_params"]["R"]
        ) * (1.0 / self.params["kint_params"]["Texp"] - 1.0 / self.params["kint_params"]["Tref"])
        lgkBexp = self.params["kint_params"]["lgkBref"] - (
            self.params["kint_params"]["EaB"] / np.log(10) / self.params["kint_params"]["R"]
        ) * (1.0 / self.params["kint_params"]["Texp"] - 1.0 / self.params["kint_params"]["Tref"])
        lgkWexp = self.params["kint_params"]["lgkWref"] - (
            self.params["kint_params"]["EaW"] / np.log(10) / self.params["kint_params"]["R"]
        ) * (1.0 / self.params["kint_params"]["Texp"] - 1.0 / self.params["kint_params"]["Tref"])
        ...
        lgkB = (
            ...
            + rate_adjs[1]
            + rate_adjs[3]
            - self.params["kint_params"]["pKD"]
            + self.params["kint_params"]["pD"]
        )
    ...
    def kint(self):
        ...
        for c in self.top.chains:
            if c.n_residues > 1:  # Chain of length 1 is probably a ligand - ignore!
                ...
                try:
                    ...
                    reslist = np.insert(
                        reslist, insert_idx, firstres
                    )  # Insert 'prev' residue for first index of each chain, as we need to define these as NT
                except ValueError:
                    pass
        oldnames = {}
        for i in reslist:
                ...
                if np.max(curr.cterm):  # If cterm is true for any frame
                    ...
                    try:
                        if curr.atom("O").n_bonds > 1 or curr.atom("OXT").n_bonds > 1:
                            ...
                            with open(self.params["logfile"], "a") as f:
                                f.write(
                                    "It looks like you have a neutral C-terminus (COOH) at residue %s?\n"
                                    % curr
                                )
                        else:
                            curr.name = "CT"
                        ...
                        with open(self.params["logfile"], "a") as f:
                            f.write(
                                "Residue %s is defined as a C-terminus but has no atom O or OXT, is this correct?\n"
                                % curr
                            )
                        curr.name = "CT"
                        continue
        ...
        try:
            if np.array_equal(reslist, self.reslist):
                ...
                raise Functions.HDX_Error(
                    "Your residue lists for protection factors and intrinsic rates are different. Check your inputs!"
                )
        except AttributeError:
        ...
        for i, r in enumerate(reslist):
            ...
            if prev.name == "PROC":
                ...
                adj_cis, adj_trans = (
                    self.params["_reordered_kint_adjs"][curr.name][0:2],
                    self.params["_reordered_kint_adjs"][curr.name][0:2],
                )
                ...
                kints[i] = np.mean(
                    kint_byframe
                )  # Overall intrinsic rate is adjusted by mean population of cis-pro
            else:
        ...
        if os.path.exists(self.params["outprefix"] + "Intrinsic_rates.dat"):
            ...
            np.savetxt(
                self.params["outprefix"] + "Intrinsic_rates_chunk_%d.dat" % (filenum + 1),
                np.stack((rids, kints), axis=1),
                fmt=["%7d", "%18.8f"],
                header="ResID  Intrinsic rate / min^-1 ",
            )  # Use residue indices internally, print out IDs
        else:
            np.savetxt(
                self.params["outprefix"] + "Intrinsic_rates.dat",
                np.stack((rids, kints), axis=1),
                fmt=["%7d", "%18.8f"],
                header="ResID  Intrinsic rate / min^-1 ",
            )  # Use residue indices internally, print out IDs
    ...
    def dfrac(self, write=True, use_self=True, alternate_pfs=None):
        ...
        if use_self:
            try:
                curr_pfs = np.stack(
                    (
                        np.exp(np.mean(self.lnpf_byframe, axis=1)),
                        np.exp(np.std(self.lnpf_byframe, axis=1, ddof=1)),
                    ),
                    axis=1,
                )
            except AttributeError:
                curr_pfs = np.stack(
                    (np.mean(self.pf_byframe, axis=1), np.std(self.pf_byframe, axis=1, ddof=1)),
                    axis=1,
                )  # No lnPF for Persson-Halle etc

            if (
                len(set(map(len, [self.reslist, self.pfs, self.rates]))) != 1
            ):  # Check that all lengths are the same (set length = 1)
                raise Functions.HDX_Error(
                    "Can't calculate deuterated fractions, your residue/protection factor/rates arrays are not the same length."
                )
        else:
            curr_pfs = alternate_pfs
            if (
                len(set(map(len, [self.reslist, alternate_pfs, self.rates]))) != 1
            ):  # Check that all lengths are the same (set length = 1)
                raise Functions.HDX_Error(
                    "Can't calculate deuterated fractions, your provided residue/protection factor/rates arrays are not the same length."
                )
        ...
        outfracs = np.reshape(
            fracs.flatten(), (fracs.shape[0], fracs.shape[1] * fracs.shape[2])
        )  # Reshape as 'Time Err Time Err...'
        if write:
            if os.path.exists(self.params["outprefix"] + "Residue_fractions.dat"):
                ...
                np.savetxt(
                    self.params["outprefix"] + "Residue_fractions_chunk_%d.dat" % (filenum + 1),
                ...
                np.savetxt(
                    self.params["outprefix"] + "Residue_fractions.dat",
                    np.concatenate((rids, outfracs), axis=1),
                    fmt="%7d " + "%8.5f " * len(self.params["times"] * 2),
