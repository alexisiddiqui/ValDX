...
class Radou(DfPred.DfPredictor):
        ...
        if isinstance(other, Radou):
            ...
            try:
                if np.array_equal(new.rates, other.rates):
                    new.pfs[:, 0] = (self.n_frames * self.pfs[:, 0]) + (
                        other.n_frames * other.pfs[:, 0]
                    )
                    # SD = sqrt((a^2 * var(A)) + (b^2 * var(B)))
                    new.pfs[:, 1] = np.sqrt(
                        (self.n_frames**2 * self.pfs[:, 1] ** 2)
                        + (other.n_frames**2 * other.pfs[:, 1] ** 2)
                    )
                    ...
                    new.lnpf_byframe = np.concatenate(
                        (self.lnpf_byframe, other.lnpf_byframe), axis=1
                    )
                    ...
                    raise Functions.HDX_Error(
                        "Cannot sum two method objects with different intrinsic rates."
                    )
    ...
    def _calc_contacts_switch(self, qidx, cidx, cutoff, scale):
        ...
        raise NotImplementedError("Switching function disabled")
    ...
    def calc_contacts(self, qidx, cidx, cutoff, scale=None):
        ...
        cmethods = {"cutoff": self._calc_contacts_cutoff, "switch": self._calc_contacts_switch}
    ...
    def _calc_hbonds_contacts(self, HN):
        ...
        if self.params["contact_method"] == "switch":
            hbond_counts = self.calc_contacts(
                HN, c, self.params["cut_Nh"], self.params["switch_scale_Nh"]
            )
        else:
    ...
    def _calc_hbonds_bh(self, HN, minfreq=0.0):
        ...
        if self.params["protonly"]:
            c = self.t.atom_slice(
                self.top.select("protein and (symbol O or symbol N) or index %s" % HN)
            )
        else:
        ...
        bond_triplets = md.geometry.hbond._get_bond_triplets(
            c.topology, exclude_water=self.params["protonly"]
        )
        mask, distances, angles = md.geometry.hbond._compute_bounded_geometry(
            c, bond_triplets, self.params["bh_dist"], [1, 2], [0, 1, 2], freq=minfreq, periodic=True
        )

        # can select distance/angle criteria here
        try:
            ang_rad = 2.0 * np.pi / (360.0 / self.params["bh_ang"])
        except ZeroDivisionError:
            self.params["bh_ang"] = 360.0
            ang_rad = 2.0 * np.pi / (360.0 / self.params["bh_ang"])

        hbond_counts = np.sum(
            np.logical_and(distances < self.params["bh_dist"], angles > ang_rad), axis=1
        )
        return hbond_counts
    ...
    def calc_hbonds(self, donors):
        ...
        hmethods = {"contacts": self._calc_hbonds_contacts, "bh": self._calc_hbonds_bh}
    ...
    def calc_nh_contacts(self, reslist):
        ...
        is_heavy = lambda _: self.top.atom(_).element.symbol != "H"
        ...
        for idx, res in enumerate(reslist):
            ...
            inv_atms = Functions.select_residxs(
                self.t, excl_idxs, protonly=self.params["protonly"], invert=True
            )  # At this stage includes H + heavys
            heavys = inv_atms[np.array([is_heavy(i) for i in inv_atms])]  # Filter out non-heavys

            if self.params["contact_method"] == "switch":
                contact_count[idx] = self.calc_contacts(
                    robj.atom("N").index,
                    heavys,
                    cutoff=self.params["cut_Nc"],
                    scale=self.params["switch_scale_Nc"],
                )
            else:
                contact_count[idx] = self.calc_contacts(
                    robj.atom("N").index, heavys, cutoff=self.params["cut_Nc"]
                )
    ...
    def PF(self):
        ...
        if prolines is not None:
            reslist = [
                self.top.atom(a).residue.index
                for a in hn_atms
                if self.top.atom(a).residue.is_protein
                and self.top.atom(a).residue.index not in prolines[:, 1]
            ]
        else:
            reslist = [
                self.top.atom(a).residue.index
        ...
        if not np.array_equal(hres, cres):
            raise Functions.HDX_Error(
                "The residues analysed for Nc and Nh appear to be different. Check your inputs!"
            )
        ...
        if self.params["save_detailed"]:
            for i, residx in enumerate(hres):
                with open(
                    "Hbonds_chain_%d_res_%d.tmp"
                    % (self.top.residue(residx).chain.index, self.top.residue(residx).resSeq),
                    "ab",
                ) as hbond_f:
                    np.savetxt(
                        hbond_f, hbonds[i], fmt=outfmt
                    )  # Use residue indices internally, print out IDs
            for i, residx in enumerate(cres):
                with open(
                    "Contacts_chain_%d_res_%d.tmp"
                    % (self.top.residue(residx).chain.index, self.top.residue(residx).resSeq),
                    "ab",
                ) as contacts_f:
                    np.savetxt(
                        contacts_f, contacts[i], fmt=outfmt
                    )  # Use residue indices internally, print out IDs
        ...
        if os.path.exists(self.params["outprefix"] + "Protection_factors.dat"):
            ...
            np.savetxt(
                self.params["outprefix"] + "Protection_factors_chunk_%d.dat" % (filenum + 1),
                np.concatenate((rids, pf_bar), axis=1),
                fmt=["%7d", "%18.8f", "%18.8f"],
                header="ResID  Protection factor Std. Dev.",
            )  # Use residue indices internally, print out IDs
        else:
            np.savetxt(
                self.params["outprefix"] + "Protection_factors.dat",
                np.concatenate((rids, pf_bar), axis=1),
                fmt=["%7d", "%18.8f", "%18.8f"],
                header="ResID  Protection factor Std. Dev.",
            )  # Use residue indices internally, print out IDs
        ...
        if os.path.exists(self.params["outprefix"] + "logProtection_factors.dat"):
            ...
            np.savetxt(
                self.params["outprefix"] + "logProtection_factors_chunk_%d.dat" % (filenum + 1),
                np.concatenate((rids, lnpf_bar), axis=1),
                fmt=["%7d", "%18.8f", "%18.8f"],
                header="ResID  ln(Protection factor) Std. Dev.",
            )  # Use residue indices internally, print out IDs
        else:
            np.savetxt(
                self.params["outprefix"] + "logProtection_factors.dat",
                np.concatenate((rids, lnpf_bar), axis=1),
                fmt=["%7d", "%18.8f", "%18.8f"],
                header="ResID  ln(Protection factor) Std. Dev.",
            )  # Use residue indices internally, print out IDs
    ...
    def run(self, trajectory):
        ...
        self.top = (
            trajectory.topology.copy()
        )  # This does not add attributes to the original topology
...
class PH(DfPred.DfPredictor):
    ...
    def calc_wat_contacts(self, hn_atms):
        ...
        for idx, hn in enumerate(hn_atms):
            contacts[idx] = np.array(
                list(
                    map(
                        len,
                        md.compute_neighbors(
                            self.t, self.params["cut_O"], np.asarray([hn]), haystack_indices=solidxs
                        ),
                    )
                )
            )
            if self.params["save_detailed"]:
                with open(
                    "Waters_chain_%d_res_%d.tmp"
                    % (self.top.atom(hn).residue.chain.index, self.top.atom(hn).residue.resSeq),
                    "ab",
                ) as wat_f:
    ...
    def PF(self, update_only=False):
        if not update_only:
            ...
            if self.params["skip_first"]:
                if not np.array_equal(protlist[1:], self.reslist):
                    raise Functions.HDX_Error(
                        "One or more residues analysed for water contacts is either proline or a non-protein residue. Check your inputs!"
                    )
            else:
        ...
        opencount, closedcount = (
            np.sum(self.watcontacts > 1, axis=1),
            np.sum(self.watcontacts <= 1, axis=1),
        )
