...
command("structure @example_open_state_prot.psf end") # might not be necessary?

num_frames = 1539
structures = [ "Cluster_1_eps_4.22e+01_bbonly.pdb.%d" % i for i in range(1,num_frames+1) ]
...
probmap.calc()
probmap.writeEDM("BBonly_cluster_1_script_flat.xplor")
# contribution from each atom will be
# normalized by the maximum contribution
# from that atom
probmap.setScaleType("amplitude")
probmap.calc()
probmap.writeEDM("BBonly_cluster_1_script_amplitude.xplor")
...
probmap.calc()
probmap.writeEDM("BBonly_cluster_1_script_normalize.xplor")
