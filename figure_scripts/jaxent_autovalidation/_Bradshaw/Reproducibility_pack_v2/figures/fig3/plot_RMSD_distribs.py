...
_weightsfiles = glob("./data_folder/mixed_gamma_*final_weights.dat")
sorted_weightsfiles = []
sorted_workfiles = []
gammas = []
for e in range(-3,4):
    for n in range(1,2):
        s = str(n) + "x10^" + str(e)
        if "./data_folder/mixed_gamma_%s_final_weights.dat" % s in _weightsfiles:
            sorted_weightsfiles.append("./data_folder/mixed_gamma_%s_final_weights.dat" % s)
            sorted_workfiles.append("./data_folder/mixed_gamma_%s_work.dat" % s)
...
rmsds = np.loadtxt("./data_folder/closed_rmsd_all.xvg", skiprows=16, dtype=[ ('idxs',np.int32,(1,)) , ('rmsd',np.float64,(1,)) ] )
# Read in RMSDs of closed and extract indices of frames with RMSD < 1
openrmsds = np.loadtxt("./data_folder/open_rmsd_all.xvg", skiprows=16, dtype=[ ('idxs',np.int32,(1,)) , ('rmsd',np.float64,(1,)) ] )
# Read in RMSDs of all and extract indices of frames with RMSD < 1.1, 1.25, 1.5, 1.75, 2.0
fullrmsds = np.loadtxt("./data_folder/neutral_rmsd_all.xvg", skiprows=16, dtype=[ ('idxs',np.int32,(1,)) , ('rmsd',np.float64,(1,)) ] )
# Read in RMSDs of all to open and extract indices of frames with RMSD < 1.1, 1.25, 1.5, 1.75, 2.0
openfullrmsds = np.loadtxt("./data_folder/neutral_rmsd_open_all.xvg", skiprows=16, dtype=[ ('idxs',np.int32,(1,)) , ('rmsd',np.float64,(1,)) ] )
...
# Plot
#xs = np.asarray(gammas)
#rmsd_cuts.insert(0,0.1)
# Axis 0 = frames, axis 1 = trajectories

# Weighted RMSD distribution
fig, axs = plt.subplots(1,2, figsize=(22,8.5), sharey=True)
bins = np.arange(0.0,0.402,0.002)*10
n, _ = np.histogram(fullrmsds['rmsd'].flatten()*10, density=True, bins=bins, weights=iniweights)
binmidp = bins[:-1]+0.01
axs[0].axvline(1.0, ls='--', c='k', ymax=0.75)
axs[0].set_prop_cycle(None) # resets to that in mplstyle
axs[0].plot(binmidp, n, label="Initial distribution", lw=4)
loggammas = np.log10(gammas).astype(int)

for gamma, w in zip(loggammas,data[1:]):
    n, _ = np.histogram(fullrmsds['rmsd'].flatten()*10, weights=w, density=True, bins=bins)
#    binmidp = bins[:-1]+0.001
    axs[0].plot(binmidp, n, label=r"Gamma 10$^{%d}$" % gamma, lw=4)
...
# Open
n, _ = np.histogram(openfullrmsds['rmsd'].flatten()*10, density=True, bins=bins, weights=iniweights)
binmidp = bins[:-1]+0.01
axs[1].axvline(1.0, ls='--', c='k', ymax=0.75)
axs[1].set_prop_cycle(None) # resets to that in mplstyle
init_line, = axs[1].plot(binmidp, n, label="Initial distribution", lw=4)
gamma_lines = []
for gamma, w in zip(loggammas,data[1:]):
    n, _ = np.histogram(openfullrmsds['rmsd'].flatten()*10, weights=w, density=True, bins=bins)
#    binmidp = bins[:-1]+0.001
    curr, = axs[1].plot(binmidp, n, label=r"Gamma 10$^{%d}$" % gamma, lw=4)
    gamma_lines.append(curr)


# Titles etc.
#fig.suptitle("RMSD distributions before & after reweighting")
axs[0].set_title("RMSD to closed TeaA structure")
axs[0].set_xlabel(r"RMSD / $\AA$")
axs[0].set_xlim(0, 4.0)
...
#axs[0].legend()
axs[1].set_title("RMSD to open TeaA structure")
axs[1].set_xlabel(r"RMSD / $\AA$")
axs[1].set_xlim(0, 4.0)
axs[1].set_xticks(np.arange(0.5,4.5,0.5))
#ax.set_xticklabels(gammas)
#axs[0].set_ylim(0,np.max(weights)*1.1)
leg_1 = axs[1].legend([init_line], ["Initial distribution"], loc=(0.275,0.91), ncol=2)
leg_2 = axs[1].legend(gamma_lines, [r"Gamma 10$^{%d}$" % gamma for gamma in loggammas], loc=(0.275,0.75), ncol=2)
...
plt.tight_layout()
plt.savefig("RMSD_weighted_distribution_mixed6040.pdf", dpi=300)
