...
folders = [ './data_folder/' + i + '/10^-6_MSD' for i in names ]
...
segs_lines = []
for name, w in zip(names,data[1:]):
    try:
        int(name) + 1
        name = name + "-residue segments"
    except ValueError:
...
# Titles etc.
#fig.suptitle("RMSD distributions before & after reweighting, effect of segment averaging")
...
#axs[0].legend()
leg_1 = ax.legend([init_line], ["No reweighting"], loc=(0.275,0.91), ncol=2)
fullnames = []
for name in names:
    try:
        int(name) + 1
        fullnames.append(name + "-residue segments")
    except ValueError:
        fullnames.append("Individual " + name)
        pass
leg_2 = ax.legend(segs_lines, fullnames, loc=(0.275,0.75), ncol=2)
...
plt.tight_layout()
plt.savefig("segment_comparison_RMSDs_closedonly.pdf", dpi=300)
