...
#    binmidp = bins[:-1]+0.001
    s, = ax.plot(binmidp, n, label=name, lw=4, c=c)
    segs_lines.append(s)
...
for name in names:
    try:
        int(name) + 1
        if int(name) == 1:
            fullnames.append("High error")
        if int(name) == 2:
            fullnames.append("Low error")
    except ValueError:
        fullnames.append("Original model")
        pass
...
plt.tight_layout()
plt.savefig("mixedmodels_comparison_RMSDs_closedonly.pdf", dpi=300)
