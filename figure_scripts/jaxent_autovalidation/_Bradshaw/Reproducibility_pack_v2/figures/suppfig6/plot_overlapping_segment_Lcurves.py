...
for d, n in zip(data,names):
    try:
        ...
        if n == "5":
            n = n + "-residue"
            ax.plot(d[:,1], d[:,2], lw=2, marker='.', markersize=6, label=n, c='cbf_ylorbr-9-seq-8')
        elif n == "10":
            n = n + "-residue"
            ax.plot(d[:,1], d[:,2], lw=2, marker='.', markersize=6, label=n, c='cbf_ylorbr-9-seq-7')
    except ValueError:
        n = "Overlapping\n10-residue"
        ax.plot(d[:,1], d[:,2], lw=2, marker='.', markersize=6, label=n, c='cbf_sky_blue')
        continue
...
plt.tight_layout()
plt.savefig("overlapping_segment_comparison_Lcurves.pdf", dpi=300)
